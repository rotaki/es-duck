...
for THREADS in $THREAD_COUNTS; do
  for RUN in $(seq 1 $BENCHMARK_RUNS); do
    ...
    LOG_FILE="${LOG_DIR}/${TOTAL_MEMORY}_${THREADS}threads_run${RUN}_${RUN_TIMESTAMP}.log"
    TEMP_OUTPUT="/tmp/clickhouse_sweep_${THREADS}_run${RUN}_${RUN_TIMESTAMP}.log"

    echo "========================================="
    echo "Running with $THREADS threads... (Run $RUN of $BENCHMARK_RUNS)"
    ...
    # Write detailed log to individual file
    {
        ...
        if [ -n "$DURATION" ]; then
            ...
            echo "Result: $TOTAL_MEMORY,$THREADS,$RUN,$DURATION"
        else
